...
import VenueGigCard from './VenueGigCard';
// import GigDetails from './GigDetails';

const VenueGigsList = ({ gigs }) => {
	...
	// }, [weights]);
	return (
		<StyledVenueGigsList className='gig-list-container'>
			{/* <p className='gigs-list-header'>Recorded weigh-ins</p> */}
			<div className='gigs-list'>
				{gigs &&
					gigs.map((gig, index) => (
						<VenueGigCard key={gig._id} gig={gig} showGigLetter={true} />
					))}
			</div>
		</StyledVenueGigsList>
	);
};
const StyledVenueGigsList = styled.div`
	/* overflow-y: scroll; */
	overflow-y: auto;
	/* flex: 1; */
