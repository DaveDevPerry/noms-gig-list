...
import CityGigCard from './CityGigCard';
// import GigDetails from './GigDetails';

const CityGigsList = ({ gigs }) => {
	...
	// }, [weights]);
	return (
		<StyledCityGigsList className='gig-list-container'>
			{/* <p className='gigs-list-header'>Recorded weigh-ins</p> */}
			<div className='gigs-list'>
				{gigs &&
					gigs.map((gig, index) => <CityGigCard key={gig._id} gig={gig} />)}
			</div>
		</StyledCityGigsList>
	);
};
const StyledCityGigsList = styled.div`
	/* overflow-y: scroll; */
	overflow-y: auto;
	flex: 1;
